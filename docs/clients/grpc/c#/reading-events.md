...
@[code{read-from-stream}](@grpc:reading-events/Program.cs)

This will return an enumerable that can be iterated on:

@[code{iterate-stream}](@grpc:reading-events/Program.cs)
...
@[code{read-from-all-stream-iterate}](@grpc:reading-events/Program.cs)
