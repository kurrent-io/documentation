...
@[code{read-from-stream}](@grpc:reading_events/ReadingEvents.java)

This will return an enumerable that can be iterated on:

@[code{iterate-stream}](@grpc:reading_events/ReadingEvents.java)
...
@[code{read-from-all-stream-iterate}](@grpc:reading_events/ReadingEvents.java)
