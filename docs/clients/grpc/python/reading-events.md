...
head:
  - - title
    - {}
    - Reading Events | Python | Clients | Kurrent Docs
---
...
@[code{read-from-stream}](@grpc:reading_events.py)

This will return an enumerable that can be iterated on:

@[code{iterate-stream}](@grpc:reading_events.py)
...
@[code{overriding-user-credentials}](@grpc:reading_events.py)
...
@[code{read-from-stream-position}](@grpc:reading_events.py)
...
@[code{reading-backwards}](@grpc:reading_events.py)
...
@[code{checking-for-stream-presence}](@grpc:reading_events.py)
...
@[code{read-from-all-stream}](@grpc:reading_events.py)

You can iterate asynchronously through the result:

@[code{read-from-all-stream-iterate}](@grpc:reading_events.py)
...
@[code{read-from-all-stream-resolving-link-Tos}](@grpc:reading_events.py)
...
@[code{read-all-overriding-user-credentials}](@grpc:reading_events.py)
...
@[code{read-from-all-stream-backwards}](@grpc:reading_events.py)
...
@[code{ignore-system-events}](@grpc:reading_events.py)
