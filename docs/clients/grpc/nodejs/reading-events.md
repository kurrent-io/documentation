...
head:
  - - title
    - {}
    - Reading Events | Node.js | Clients | Kurrent Docs
---
...
@[code{read-from-stream}](@grpc:reading-events.js;reading-events.ts)

This will return an enumerable that can be iterated on:

@[code{iterate-stream}](@grpc:reading-events.js;reading-events.ts)
...
@[code{overriding-user-credentials}](@grpc:reading-events.js;reading-events.ts)
...
@[code{read-from-stream-position}](@grpc:reading-events.js;reading-events.ts)
...
@[code{reading-backwards}](@grpc:reading-events.js;reading-events.ts)
...
@[code{checking-for-stream-presence}](@grpc:reading-events.js;reading-events.ts)
...
@[code{read-from-all-stream}](@grpc:reading-events.js;reading-events.ts)

You can iterate asynchronously through the result:

@[code{read-from-all-stream-iterate}](@grpc:reading-events.js;reading-events.ts)
...
@[code{read-from-all-stream-resolving-link-Tos}](@grpc:reading-events.js;reading-events.ts)
...
@[code{read-all-overriding-user-credentials}](@grpc:reading-events.js;reading-events.ts)
...
@[code{read-from-all-stream-backwards}](@grpc:reading-events.js;reading-events.ts)
...
@[code{ignore-system-events}](@grpc:reading-events.js;reading-events.ts)
