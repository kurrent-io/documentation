...
head:
  - - title
    - {}
    - Catch-up Subscriptions | Node.js | Clients | Kurrent Docs
---
...
@[code{subscribe-to-stream}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{subscribe-to-all}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{subscribe-to-stream-from-position}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{subscribe-to-all-from-position}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{subscribe-to-stream-live}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)

And the same works with `$all` :

@[code{subscribe-to-all-live}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
When reading a stream you can specify whether to resolve link-to events. By default, link-to events are not resolved. You can change this behaviour by setting the `resolveLinkTos` parameter to `true`:

@[code{subscribe-to-stream-resolving-linktos}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{subscribe-to-stream-subscription-dropped}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{overriding-user-credentials}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{stream-prefix-filtered-subscription}](@grpc:subscribing-to-streams.js;subscribing-to-streams.ts)
...
@[code{exclude-system}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{event-type-prefix}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{event-type-regex}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{stream-prefix}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{stream-regex}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{checkpoint}](@grpc:server-side-filtering.js;server-side-filtering.ts)
...
@[code{checkpoint-with-interval}](@grpc:server-side-filtering.js;server-side-filtering.ts)
